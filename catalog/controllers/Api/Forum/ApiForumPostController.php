...
class ApiForumPostController extends MY_Controller
{
    public function store()
    {
        ...
        if ($validator->run() == false) {
            ...
            $post = $this->ForumPostModel->create($this->input->post());
            $response = array(
                'success' => true,
                'message' => 'Đã thêm chủ đề mới',
                'post' => $post
            );
    ...
    public function update($postId)
    {
        ...
        if ($validator->run() == false) {
            ...
            $this->load->model('Forum/ForumPostModel');
            $post = $this->ForumPostModel->update($this->input->post(), $postId);
            $response = array(
                'success' => true,
                'message' => 'Đã lưu',
                'post' => $post
    ...
    {
        $this->load->model('Forum/ForumPostModel');
        $post = $this->ForumPostModel->delete($postId);
        ...
        if (!$post) {
            $response = array(
                'success' => false,
                'message' => $this->ForumPostModel->getErrors(),
            );
            $this->output->set_status_header(406);
        } else {
            $response = array(
    ...
    protected function validatorStore(array $data)
    {
        ...
        $this->form_validation->set_rules('name', 'Tên chủ đề', 'required|max_length[255]');
        $this->form_validation->set_rules('content', 'Nội dung', 'required|max_length[255]');
        $this->form_validation->set_rules('status', 'Trạng thái', 'required|integer|in_list[0,1]');
        $this->form_validation->set_rules('author_id', 'Tác giả', 'integer');
        $this->form_validation->set_rules('category_id', 'Danh mục', 'integer');
        $this->form_validation->set_rules('blog_post_id', 'Bài viết', 'integer');
