...
class ApiBlogPostController extends MY_Controller
{
    public function store()
    {
        ...
        if ($validator->run() == false) {
            ...
            $post = $this->BlogPostModel->create($this->input->post());
            $response = array(
                'success' => true,
    ...
    public function update($postId)
    {
        ...
        if ($validator->run() == false) {
            ...
            $this->load->model('Blog/BlogPostModel');
            $post = $this->BlogPostModel->update($this->input->post(), $postId);
            $response = array(
                'success' => true,
                'message' => 'Đã lưu',
                'post' => $post
    ...
    {
        $this->load->model('Blog/BlogPostModel');
        $post = $this->BlogPostModel->delete($postId);
        ...
        if (!$post) {
            $response = array(
                'success' => false,
                'message' => $this->BlogPostModel->getErrors(),
            );
            $this->output->set_status_header(406);
        } else {
            $response = array(
