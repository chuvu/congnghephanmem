...
class ApiBlogCategoryController extends MY_Controller
{
    public function store()
    {
        ...
        if ($validator->run() == false) {
            ...
            $category = $this->BlogCategoryModel->create($this->input->post());
            $response = array(
                'success' => true,
    ...
    public function update($categoryId)
    {
        ...
        if ($validator->run() == false) {
            ...
            $this->load->model('Blog/BlogCategoryModel');
            $category = $this->BlogCategoryModel->update($this->input->post(), $categoryId);
    ...
    {
        $this->load->model('Blog/BlogCategoryModel');
        $category = $this->BlogCategoryModel->delete($categoryId);
        ...
        if (!$category) {
            $response = array(
                'success' => false,
                'message' => $this->BlogCategoryModel->getErrors(),
            );
            $this->output->set_status_header(406);
        } else {
            $response = array(
    ...
    protected function validatorStore(array $data)
    {
        ...
        $this->form_validation->set_rules('Danh mục cha', 'parent_id', 'integer|exists[blog_categories,id]');
