...
class ApiProductPostController extends MY_Controller
{
    public function store()
    {
        ...
        if ($validator->run() == false) {
            ...
            $post = $this->ProductPostModel->create($this->input->post());
            $response = array(
                'success' => true,
    ...
    public function update($postId)
    {
        ...
        if ($validator->run() == false) {
            ...
            $this->load->model('Product/ProductPostModel');
            $post = $this->ProductPostModel->update($this->input->post(), $postId);
            $response = array(
                'success' => true,
                'message' => 'Đã lưu',
                'post' => $post
    ...
    {
        $this->load->model('Product/ProductPostModel');
        $post = $this->ProductPostModel->delete($postId);
        ...
        if (!$post) {
            $response = array(
                'success' => false,
                'message' => $this->ProductPostModel->getErrors(),
            );
            $this->output->set_status_header(406);
        } else {
            $response = array(
