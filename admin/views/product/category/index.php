    ...
    <div class="page-header">
        <div class="container">
            <div class="pull-right">
                <?php if (can('product.category.create')) : ?>
                    <a id="btn-new-folder" href="<?php echo url('product/category/' . $category->id . '/post/create') ?>" class="btn btn-primary"><i class="fa fa-folder-open"></i> Danh mục mới</a>
                <?php endif ?>
                <?php if (can('product.post.create')) : ?>
                    <a href="<?php echo url('product/category/' . $category->id . '/post/create') ?>" class="btn btn-primary"><i class="fa fa-file-o"></i> Thêm sản phẩm mới</a>
                <?php endif ?>
            </div>
            ...
            <tbody>
                ...
                <?php foreach ($categories as $item) : ?>
                    <tr data-category='<?php echo json_encode($item, JSON_HEX_APOS) ?>'>
                        ...
                        <td class="category-name">
                            <a href="<?php echo url('product/category/' . $item->id) ?>"><strong><i class="fa fa-cart-plus"></i>  <span class="name"><?php echo $item->name ?></span></strong></a>
                        </td>
                ...
                <?php foreach ($posts as $item) : ?>
                    <tr data-post='<?php echo json_encode($item, JSON_HEX_APOS) ?>'>
                        <td>#<?php echo $row ?></td>
                        <td><strong><?php echo $item->id ?></strong></td>
                        <td><a href="<?php echo url('product/post/'. $item->id .'/edit/') ?>"><strong><i class="fa fa-file-o"></i> <?php echo $item->name ?></strong></a></td>
                        <td>Bài viết</td>
                        <td>
                        ...
                        <td class="text-right">
                            <?php if (can('product.post.edit')) : ?>
                                <a href="<?php echo url('product/post/'. $item->id .'/edit/') ?>" class="btn btn-primary btn-sm"><i class="fa fa-pencil"></i></a>
                            <?php endif ?>
                            <?php if (can('product.post.destroy')) : ?>
    ...
    $(function () {
        ...
        <?php if (can('product.category.edit')) : ?>
            $('.post-status').on('change', function (e) {
                ...
                $.ajax({
                    type: 'post',
                    dataType: 'json',
                    url: 'api/product/post/' + post.id + '/update',
                    data: {
                        status: value
                    },
        ...
        <?php if (can('product.category.destroy')) : ?>
            $('.btn-remove-category').on('click', function (e) {
                ...
                $.ajax({
                    type: 'post',
                    url: 'api/product/category/' + category.id + '/destroy',
                    success: function () {
        ...
        <?php if (can('product.post.destroy')) : ?>
            $('.btn-remove-post').on('click', function (e) {
                ...
                $.ajax({
                    type: 'post',
                    url: 'api/product/post/' + post.id + '/destroy',
                    success: function () {
                        toastr.success('Đã xóa', 'Đã thực hiện');
                        $tr.fadeOut(function () {
                            $(this).remove();
        ...
        <?php if (can('product.category.edit')) : ?>
            $(document).off('click').on('click', '.btn-category-save', function (e) {
                ...
                $.ajax({
                    type: 'post',
                    url: 'api/product/category/' + category.id +'/update',
                    data: {
                        name: categoryName,
                    },
                    complete: function () {
